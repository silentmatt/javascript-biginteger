...
BigInteger.parse = function(s, base) {
	...
	if (parts) {
		...
		var s = sign == '-' ? -1n : 1n;
		if (base == 10) {
			return new BigInteger(s * BigInt(digits), CONSTRUCT);
		} else if (base === 2) {
			return new BigInteger(s * BigInt('0b' + digits), CONSTRUCT);
		} else if (base === 8) {
			...
			return new BigInteger(s * BigInt('0x' + digits), CONSTRUCT);
		} else {
			// Do the conversion
			var d = 0n;
			...
			return new BigInteger(s * d, CONSTRUCT);
		}
	}
	else {
...
BigInteger.prototype.divRemSmall = function(n) {
	...
	return [
		new BigInteger(BigInt(sign) * (this.abs().value / n), CONSTRUCT),
		new BigInteger(BigInt(sign) * (this.abs().value % n), CONSTRUCT)
	];
};
